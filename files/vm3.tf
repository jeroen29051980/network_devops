...
resource "time_sleep" "wait {
  depends_on = [oci_core_instance.BACKUP_VM]
  create_duration   = "60s"
}
...
resource "null_resource" "generate-inventory" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      echo ${oci_core_instance.BACKUP_VM.display_name} ansible_host=${oci_core_instance.BACKUP_VM.public_ip} ansible_user=opc ansible_ssh_private_key_file=/tmp/sshkey >> inventory
    EOT
  }
